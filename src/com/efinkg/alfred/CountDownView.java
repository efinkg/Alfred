 ...
 * 
 * The current animation for each second is as follow: 1. From 0 to 500ms, move
 * the TextView from {@code MAX_TRANSLATION_Y} to 0 and its alpha from {@code 0}
 * to {@code ALPHA_DELIMITER}. 2. From 500ms to 1000ms, update the TextView's
 * alpha from {@code ALPHA_DELIMITER} to {@code 1}. At each second change,
...
public class CountDownView extends FrameLayout {
	...
	private void updateView(long millisUntilFinish) {
		long currentTimeSeconds = TimeUnit.MILLISECONDS
		...
		if (frame <= ANIMATION_DURATION_IN_MILLIS) {
			...
			mSecondsView.setAlpha(ALPHA_DELIMITER + factor
					* (1 - ALPHA_DELIMITER));
		}
	}
